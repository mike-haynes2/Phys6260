...
# 
def initialize(N):
    # Initialize a uniform distribution in a cube centered on the origin 
    # and then restrict to sphere.  A cube that bounds a sphere has 
    # about twice the volume.  So to be on the safe side (because randomness), 
    # I generate `3*Npart` particle positions and then select the
    ...
    # Circular radius of a cross-section (xy-plane) of the sphere at 
    # coordinate z of the particle. This is the particle's initial 
    # orbit for a solid body rotator. We need this to calculate 
    # the x- and y-components of the initial velocity, setting vz = 0.
...
def calc_accel(comm, pos):
    ...
    ### all other processes
    # init a blank array to return, pupulated
    all_accel = np.zeros([Npart,3])
    num_procs = comm.size
    inc = Npart/num_procs
    # gather the subsections of the array from each process

    all_accel_gathered = comm.gather(accel, root=0)
    all_accel_gathered = comm.bcast(all_accel_gathered,root=0)
    # assign into each proc's array based on the sequential distribution in subrange()
...
myPE=comm.rank 
# use the broadcast function to do a MPI_SEND operation concurrently with each available processor
pos = comm.bcast(pos, root=0)
vel = comm.bcast(vel, root=0)
print(str(myPE)+' '+str(np.shape(pos)))
...
while _time < tstop:
    ...
    istep += 1
    _time += dt
    if (comm.rank == 0) and (istep % (nstep/1000) == 0 or istep == 1):
        plt.clf()
